...
class fF(object):
    ...
    def __init__(self, reader_object, option_object):
        ...
        self.fun_dict = {"single_objective": self.single_objective_fitness,
                        "single_objective_inspyred":self.single_objective_fitness_inspyred,
...
#        print fsum(tmp)/len(tmp)
    ...
    def single_objective_fitness_inspyred(self, candidates, args={}, delete_model=True):
        """
        ...
        for l in candidates:
            ...
            with open("eval.txt", "a") as f: 
                f.write(str([temp_fit,[x for x in candidates[0]]])+" \n")
            if self.option.output_level == "1":
        ...
        for k in range(k_range):     #for k in range(self.reader.number_of_traces()):
            ...
            if (not self.modelRunner(l,k)):
                ...
                if (self.option.type[-1]!='features'):
                    ...
                    temp_fit=100
        with open("eval.txt", "a") as f: 
            f.write(str([temp_fit,[x for x in candidates]])+" \n")
        if self.option.output_level == "1":
            print("current fitness: ",temp_fit)
        if(self.option.simulator == 'Neuron') and delete_model:
            "Deletes the reference of the instance"
            ...
            for k in range(k_range):     #for k in range(self.reader.number_of_traces()):
                ...
                if (not self.modelRunner(l,k)):
                    ...
                    if (self.option.type[-1]!='features'):
                        ...
                        temp_fit.append(100)
            self.fitnes=[[x for x in temp_fit]]
            with open("eval.txt", "a") as f: 
                f.write(str([temp_fit,[x for x in candidates[0]]])+" \n")
            if self.option.output_level == "1":
                print("current fitness: ",temp_fit)
